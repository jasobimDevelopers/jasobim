...
public class ExportExcelOfMenchainc {
	public void getValue(List<MechanicPricePojos> userList,FileOutputStream fout,String projectName,Integer month){
		  try{
	            //1.创建工作簿
	          ...
	          //1.1创建合并单元格对象
	            ...
	            if(userList != null)
	            {
	                for(int j=0;j<userList.size();j++)
	                {
	                    ...
	                    cell5s.setCellValue(userList.get(j).getSalaryNum());
	                }
	            }
	            //5.输出
	            workbook.write(fout);
