...
public class RedisTest {
    ...
    public void setUp() {
    	...
    	pool = new JedisPool(new JedisPoolConfig(),"139.224.59.3");
...
*/
    ...
    public void testList(){
        ...
        //开始前，先移除所有的内容
        /*jedis.del("testss");
        System.out.println(jedis.lrange("testss",0,-1));*/
       //先向key java framework中存放三条数据
       jedis.lpush("testss1","spring1");
       jedis.lpush("testss1","struts1");
       jedis.lpush("testss1","hibernate1");
       //再取出所有数据jedis.lrange是按范围取出，
       // 第一个是key，第二个是起始位置，第三个是结束位置，jedis.llen获取长度 -1表示取得所有
       System.out.println(jedis.lrange("testss1",0,-1));
    }
    ...
    @Test
    public void testCommMethod() throws InterruptedException {
        ...
        jedis.setex("timekey", 1000, "min");//通过此方法，可以指定key的存活（有效时间） 时间为秒
        //Thread.sleep(5000);//睡眠5秒后，剩余时间将为<=5
        System.out.println(jedis.ttl("timekey"));   //输出结果为5
        //jedis.setex("timekey", 1, "min");        //设为1后，下面再看剩余时间就是1了
        System.out.println(jedis.ttl("timekey"));  //输出结果为1
        System.out.println(jedis.exists("key"));//检查key是否存在
        System.out.println("timekey=========="+jedis.exists("timekey"));
        
        jedis.set("aaa", "aaaVlaue");
        System.out.println(jedis.rename("aaa","aaam"));
        System.out.println("aaam========="+jedis.get("aaam"));
